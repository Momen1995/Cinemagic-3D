 ...
 const movieElement = document.querySelector(".movies");
  ...
  for(let i = 0 ; i < moviesArr.length ; i++){
    ...
    <div class="movie-poster">
        <img src="${moviesArr[i].posterURL}" class="card-img-top w-100" alt="...">
      </div>
      <div class="card-body">
        <h5 class="card-title fw-bold">${moviesArr[i].title} (${moviesArr[i].year})</h5>
        <h5 class="fw-bold text-danger">${moviesArr[i].ticketPrice}$</h5>
        <button class="btn btn-primary fw-bold book-ticket">Book Ticket</button>
      </div> 
    `;

    movieElement.appendChild(movie)
  }
}
